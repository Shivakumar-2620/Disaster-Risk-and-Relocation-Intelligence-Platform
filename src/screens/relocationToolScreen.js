    ...
    <div class="p-4 md:p-margin-desktop max-w-7xl mx-auto flex flex-col gap-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
          ...
          <div class="bg-surface-container-low p-4 rounded-xl border border-outline-variant space-y-2">
            <div class="flex justify-between font-semibold">
              ...
              <span id="weight-geo-val" class="text-primary font-bold font-mono">${mcdaWeights.geological}%</span>
            </div>
            <input type="range" id="weight-geo" min="10" max="60" value="${mcdaWeights.geological}" class="w-full accent-primary">
          ...
          <div class="bg-surface-container-low p-4 rounded-xl border border-outline-variant space-y-2">
            <div class="flex justify-between font-semibold">
              ...
              <span id="weight-dist-val" class="text-primary font-bold font-mono">${mcdaWeights.distance}%</span>
            </div>
            <input type="range" id="weight-dist" min="10" max="50" value="${mcdaWeights.distance}" class="w-full accent-primary">
          ...
          <div class="bg-surface-container-low p-4 rounded-xl border border-outline-variant space-y-2">
            <div class="flex justify-between font-semibold">
              ...
              <span id="weight-cost-val" class="text-primary font-bold font-mono">${mcdaWeights.cost}%</span>
            </div>
            <input type="range" id="weight-cost" min="10" max="50" value="${mcdaWeights.cost}" class="w-full accent-primary">
          ...
          <div class="bg-surface-container-low p-4 rounded-xl border border-outline-variant space-y-2">
            <div class="flex justify-between font-semibold">
              ...
              <span id="weight-util-val" class="text-primary font-bold font-mono">${mcdaWeights.utility}%</span>
            </div>
            <input type="range" id="weight-util" min="10" max="50" value="${mcdaWeights.utility}" class="w-full accent-primary">
      ...
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6" id="candidate-sites-container">
          ${calculatedSites.map((site, index) => {
...
export function setupRelocationToolEvents() {
  const geoInput = document.getElementById('weight-geo');
  const distInput = document.getElementById('weight-dist');
  const costInput = document.getElementById('weight-cost');
  const utilInput = document.getElementById('weight-util');

  function handleWeightChange() {
    const weights = {
      geological: parseInt(geoInput?.value || 35, 10),
      distance: parseInt(distInput?.value || 25, 10),
      cost: parseInt(costInput?.value || 20, 10),
      utility: parseInt(utilInput?.value || 20, 10)
    };

    appState.setMCDAWeights(weights);

    document.getElementById('weight-geo-val').textContent = `${weights.geological}%`;
    document.getElementById('weight-dist-val').textContent = `${weights.distance}%`;
    document.getElementById('weight-cost-val').textContent = `${weights.cost}%`;
  ...
  geoInput?.addEventListener('input', handleWeightChange);
  distInput?.addEventListener('input', handleWeightChange);
  costInput?.addEventListener('input', handleWeightChange);
  utilInput?.addEventListener('input', handleWeightChange);
